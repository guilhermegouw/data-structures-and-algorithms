...
which runs of data are stored as single data value and count, rather than as
...
sorts of special characters, including numbers. And sinse encoded data must be
decodable, this means that we can't naively run-length-encode long runs.
For example: the run "AAAAAAAAAAAA" (12As), can't naively be encoded as "12A",
